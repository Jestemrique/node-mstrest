...
module.exports = class TaskConnection {
    ...
    /**
     * @param {Sting} newUrl - URL pointing to MicroStrategy Library REST API base
     */
    setBaseUrl(newUrl) {
    ...
    gerSessionState(){
        if (!this.sessionState) {
            throw new Error('No stored serssion state');
        }
     ...
     * 
     * @param {String} taskId
     * @param {Object} [parameters={}] Key:valur list of parameters
     * @param {String} [taskEnv='xhr']
     * @param {String} [taskContentType='json']
     * @param {String} (optional) Override the URL where the task API reuest goes. Defaults to taskProcUrl defined on class init.
     * @returns {Promise} Parsed response
     * 
     */
    _executeTask(taskId, parameters = {}, taskEnve = 'xhr', taskContentType = 'json', taskProcUrl = this.getBaseUrl()) {
